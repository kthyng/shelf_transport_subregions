...
Plotted here is a combination of five subsets.
'''
...
whichseason = 'summer' # 'winter' or 'summer' for interannual
...
if whichtime == 'seasonal':
    ...
    for i, ax in enumerate(axarr):
        ...
        if i==0:
            ...
            ax.set_title('Winter')
            Files = glob.glob('CH/tracks/20??-0[1,2]-*gc.nc')
            Files.extend(glob.glob('LA/tracks/20??-0[1,2]-*gc.nc'))
            Files.extend(glob.glob('MX/tracks/20??-0[1,2]-*gc.nc'))
            Files.extend(glob.glob('NTX/tracks/20??-0[1,2]-*gc.nc'))
            Files.extend(glob.glob('STX/tracks/20??-0[1,2]-*gc.nc'))
        elif i==1:
            tracpy.plotting.background(grid=grid, ax=ax, parslabels=[0,0,0,0], mers=np.arange(-100, -80, 2))
            ax.set_title('Summer')
            Files = glob.glob('CH/tracks/20??-0[7,8]-*gc.nc')
            Files.extend(glob.glob('LA/tracks/20??-0[7,8]-*gc.nc'))
            Files.extend(glob.glob('MX/tracks/20??-0[7,8]-*gc.nc'))
            Files.extend(glob.glob('NTX/tracks/20??-0[7,8]-*gc.nc'))
            Files.extend(glob.glob('STX/tracks/20??-0[7,8]-*gc.nc'))
        ...
        var = (S[i,:,:]/Smax)
        if howplot=='log':
            # lev_exp = np.linspace(np.log10(0.1),np.log10(1.0),100)
            lev_exp = np.linspace(np.log10(0.01),np.log10(0.1),100)
            levs = np.power(10, lev_exp)
            ind = var>0.1
            var[ind] = 0.1
            mappable = ax.contourf(grid['xpsi'], grid['ypsi'], var, cmap=cmap, levels=levs, norm=colors.LogNorm())
        elif howplot=='linear':
            # levs = np.linspace(0,1,100)
            levs = np.linspace(0,0.1,100)
            mappable = ax.contourf(grid['xpsi'], grid['ypsi'], var, cmap=cmap, levels=levs, extend='max')

        # ax.contour(grid['xpsi'], grid['ypsi'], var, [0.9], colors='0.2')
        ...
        loc_shelftransport = '/home/kthyng/projects/shelf_transport/'
        LA = np.load(loc_shelftransport + 'calcs/LApts.npz')['LA']
        xp = LA[:,0]; yp = LA[:,1]
        ax.plot(xp, yp, 'k', lw=2, alpha=0.5)
        loc_shelftransport = '/home/kthyng/projects/shelf_transport/'
        MX = np.load(loc_shelftransport + 'calcs/MXpts.npz')['MX']
        xp = MX[:,0]; yp = MX[:,1]
        ax.plot(xp, yp, 'k', lw=2, alpha=0.5)
        loc_shelftransport = '/home/kthyng/projects/shelf_transport/'
